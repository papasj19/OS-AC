...
void *doMedian(void *arg) {
    ...
    for (int i = 0; i < size; i++) {
        arrCopy[i] = info[i];  // Fix: Copy from info[i], not info[i + 1]
    }

    // Sort the copied array
    qsort(arrCopy, size, sizeof(float), compare);  // Assuming compare() is defined elsewhere

    // Always return the middle element, regardless of even/odd size
    float median = arrCopy[size / 2];  // Always return the middle element

    // Allocate memory for the return value
    float *returnable = malloc(sizeof(float));  // Cleaner malloc call
    if (returnable == NULL) {
        perror("Malloc failed");
