...
#define NUM_EVENTS 9        // Not using 10 because we if so there are 11 events per station. IDK
...
// Shared memory and pipe setup
int shm_id;
int *passenger_counts;
int pipes[NUM_STATIONS][2]; // Each station has a dedicated pipe
...
void close_all_pipes(int station_id) {
    for (int i = 0; i < NUM_STATIONS; i++) {
        if (i != station_id) { // Don't close station's own pipe in child process
            close(pipes[i][0]);
            close(pipes[i][1]);
        }
    }
...
void station_process(int station_id) {
    ...
    srand(time(NULL) ^ (station_id << 8)); // Seed random number generator uniquely per station
    ...
    for (int i = 0; i < NUM_EVENTS; i++) {
        ...
        // Update passenger count in shared memory
        ...
        // Write report to Control Center through pipe
        write(pipes[station_id][1], &report, sizeof(Report));
        
        usleep((rand() % 1000) * 1000); // Random delay from 0 to 1000ms --> 1s :D
    }

    // Send an END_REPORT message
    Report end_report = {END_REPORT, station_id, 0};
    write(pipes[station_id][1], &end_report, sizeof(Report));
    
    close(pipes[station_id][1]); // Close write end for this station
    exit(EXIT_SUCCESS);
}
...
void control_center() {
    ...
    for (int i = 0; i < NUM_STATIONS; i++) {
        close(pipes[i][1]); // Close write ends for each pipe in parent process
    }
    ...
    while (stations_active > 0) {
        ...
        // Add each station's read-end to fd_set
        for (int i = 0; i < NUM_STATIONS; i++) {
            if (pipes[i][0] != -1) { // Only add active pipes
                FD_SET(pipes[i][0], &read_fds);
                if (pipes[i][0] > max_fd) max_fd = pipes[i][0];
            }
        }
        
        // Wait for reports from any station
        if (select(max_fd + 1, &read_fds, NULL, NULL, NULL) == -1) {
            handle_error("select");
        }
        
        // Check which station has sent data
        for (int i = 0; i < NUM_STATIONS; i++) {
            if (pipes[i][0] != -1 && FD_ISSET(pipes[i][0], &read_fds)) {
                ...
                if (bytes_read == 0) {
                    // Close pipe if end of transmission is detected
                    close(pipes[i][0]);
                    pipes[i][0] = -1; // Mark as closed
                    stations_active--;
                } else {
                    // Display report using file descriptor
                    char buffer[128];
...
int main() {
    ...
    // Initialize passenger counts to zero
    ...
    // Fork station processes
    for (int i = 0; i < NUM_STATIONS; i++) {
        pid_t pid = fork();
        if (pid == -1) handle_error("fork");
        if (pid == 0) {
            station_process(i); // Child process executes station simulation
        }
    }

    // Control Center manages reports
    control_center();
